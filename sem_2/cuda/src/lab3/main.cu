...
int main() {   // G:\Projects\CUDA\lab3\original.bin    G:\Projects\CUDA\lab3\result.bin
...
//    std::cin >> fileName2;
    FILE *fp;
    if ((fp = fopen("G:\\Projects\\CUDA\\lab3\\1.bin", "rb")) == NULL) {
        fprintf(stderr, "ERROR in %s:%d. Message: %s\n",
...
//        int k = i+25;
    ...
    for (int i = blocks_min; i <= blocks_max; i *= 2) {
        for (int j = threads_min; j <= threads_max; j *= 2) {
            ...
            printf("blocks(%d)-threads(%d) = %d \n", i,j,ellapsed_seconds);
        }
    }
    ...
    if ((fp = fopen("G:\\Projects\\CUDA\\lab3\\result.bin", "wb")) == NULL) {
        fprintf(stderr, "ERROR in %s:%d. Message: %s\n",
                __FILE__, __LINE__, "File is not open");
        exit(0);
    }
