...
object IrisExample extends App {
  ...
  val avgPetalLength = squares.sum / squares.size
  println(avgPetalLength)

  //get average petal square for flowers with sepal width > 4
//  private val squaresWithFilter = flowers
//    .filter(iris => iris.sepalWidth > 4)
//    .map(value => (value.petalWidth * value.petalLength))
  var tuple = flowers.filter(iris => iris.sepalWidth > 4)
    .foldLeft(Tuple2[Int, Int](0,0)) { (accamulator, iris) =>
      (accamulator._1 + (iris.petalWidth * iris.petalLength).toInt, accamulator._2+1)
  }
//  val avgPetalSquare = squaresWithFilter.sum / squaresWithFilter.size
  val avgPetalSquare = tuple._1 / tuple._2
